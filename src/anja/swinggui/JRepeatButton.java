...
//****************************************************************
{
    ...
    private Thread _thread; // Thread, der das wiederholte "Anklicken"
                            // des Buttons durchfuehrt

    private boolean _is_running;   // true <==> Thread laeuft

    private boolean _stop_request; // true signalisiert dem laufenden
                                   // Thread, dass er zu beenden ist.
    ...
    /**
     * Create a new JRepeatButton with label and set delay time
    ...
    //************************************************************
    // public methods which overwrite methods of the superclass
    //************************************************************
     ...
     *
     * Durch Loslassen der Maustaste oder Verlassen des Buttons
     * wird der Thread wieder beendet.
     */
        ...
        if( ! e.isMetaDown() )      // linke Maus-Taste
        {
            switch( e.getID() )
            {
              case MouseEvent.MOUSE_PRESSED:
                ...
                if( ! _is_running )
                {
                    // Es laeuft kein Thread, also wird einer gestartet.
     ...
     * Druecken bis zum Loslassen der linken Maustaste.
     * Wenn bei der Verarbeitung eines dieser ActionEvents durch einen
     ...
     * gedrueckt ist.
     *
     * Um dies zu verhindern, muss der "Click"-Thread also beim Oeffnen
     * eines modalen Dialogs beendet werden.
     * Da in diesem Fall der Fokus verloren geht, ist diese Methode
     * ein geeigneter Ort um dies ggf. zu tun.
     */
    ...
    //************************************************************
    // public methods for the implemented interfaces
    //************************************************************
        ...
        if( Thread.currentThread() == _thread )
        {
            ...
            while( ! _stop_request && isEnabled() )
            {
                ...
                try
                {
                    ...
                    // period sollte aber nicht kleiner als eine 50stel
    ...
    /**
     * Beenden des Threads, falls einer laeuft
     */
    ...
    {
        if( ! _is_running )
            return; // Es laeuft kein Thread, also nichts zu tun.

        _stop_request = true;

        _thread.interrupt(); // Falls der Thread gerade schlaeft, wird
                             // er dadurch geweckt.
    } // _stop
